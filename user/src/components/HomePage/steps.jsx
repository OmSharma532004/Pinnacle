      ...
      <div className='flex flex-col items-center justify-around text-yellow-300 py-12 w-full'>
        ...
        <div className="hidden w-[80%] md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {services.map((step, index) => (
            <div
              key={index}
              className="p-2 sm:p-4 border hover:bg-gray-300 border-purple-900 transition-all duration-200 hover:scale-105 bg-white text-purple-900 rounded-lg shadow-lg cursor-pointer"
        ...
        <div className="w-[80%] md:hidden lg:hidden">
          <Slider {...settings}>
            {services.map((step, index) => (
              <div
                key={index}
                className="p-2 sm:p-4 border bg-white text-purple-900 border-gray-200 rounded-lg shadow-lg cursor-pointer"
                onClick={() => openModal(step)}
              >
